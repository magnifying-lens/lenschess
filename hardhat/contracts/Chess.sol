...
contract Chess {
    ...
    */
    ...
    function checkGameFromStart(
        uint16[] memory moves
    ) public pure returns (uint8, uint256, uint32, uint32, uint32) {
        ...
        unchecked {
            ...
            if (moves[moves.length - 1] == accept_draw_const) {
                ...
                for (uint256 i = 0; i < moves.length; i++) {
                    (gameState, opponentState, playerState, halfMoveClock) = verifyExecuteMove(
                        gameState,
                        moves[i],
                        playerState,
                        opponentState,
                ...
                newHalfMoveClock = halfMoveClock;
        ...
        )
    {
        unchecked {
            ...
            if (fromType == pawn_const) {
                (newGameState, newPlayerState, captured) = verifyExecutePawnMove(
                    gameState,
                    fromPos,
                    toPos,
                    (uint8)(move >> 12),
                ...
                (newGameState, captured) = verifyExecuteKnightMove(
                    gameState,
                    fromPos,
                    toPos,
                    currentTurnBlack
                ...
                (newGameState, captured) = verifyExecuteBishopMove(
                    gameState,
                    fromPos,
                    toPos,
                    currentTurnBlack
                ...
                (newGameState, captured) = verifyExecuteRookMove(
                    gameState,
                    fromPos,
                    toPos,
                    currentTurnBlack
                ...
                (newGameState, captured) = verifyExecuteQueenMove(
                    gameState,
                    fromPos,
                    toPos,
                    currentTurnBlack
                ...
                (newGameState, newPlayerState, captured) = verifyExecuteKingMove(
                    gameState,
                    fromPos,
                    toPos,
                    currentTurnBlack,
            ...
            // reset halfmoveclock to 0 if pawn was moved or if any piece was captured
            if (fromType == pawn_const || captured) {
                newHalfMoveClock = 0;
            } else { // otherwise increment halfmoveclock
    ...
    function verifyExecutePawnMove(
        ...
        uint32 playerState,
        uint32 opponentState
    ) public pure returns (uint256 newGameState, uint32 newPlayerState, bool captured) {
        unchecked {
        newPlayerState = playerState;
        captured = false;
        ...
        if (currentTurnBlack != (toPos < fromPos)) {
            ...
            return (invalid_move_constant, 0x0, false);
        }
        uint8 diff = (uint8)(
        ...
        if (diff == 8 || diff == 16) {
            ...
            captured = true; // in case of successful diagonal capture or en passant
        } else return (invalid_move_constant, 0x0, false);
    ...
    function verifyExecuteKnightMove(
        ...
        uint8 toPos,
        bool currentTurnBlack
    ) public pure returns (uint256 newGameState, bool captured) {
        unchecked {
            captured = false;
            uint8 pieceToPosition = pieceAtPosition(gameState, toPos);
            if (pieceToPosition > 0) {
                if (
                    ...
                    return (invalid_move_constant, false);
                }
                captured = true;
            }
        ...
        unchecked {
            ...
            if (pieceToPosition > 0) {
                if (
                    ...
                    return (invalid_move_constant, newPlayerState, false);
                }
                captured = true;
            }
            ...
            uint8 v = getVerticalMovement(fromPos, toPos);
            if ((h <= 1) && (v <= 1)) {
                return (commitMove(gameState, fromPos, toPos), newPlayerState, captured);
            } else if ((h == 2) && (v == 0)) {
            ...
            return (invalid_move_constant, 0x00, false);
        }
    }
    ...
    function pieceUnderAttack(
        ...
        // original contract was developed for solidity < 0.8.0
        unchecked {
